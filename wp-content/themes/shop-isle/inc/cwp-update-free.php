...
function shop_isle_theme_updater() {


    $theme_data =   wp_get_theme(basename(get_template_directory() ));
    $edd_updater = new shop_isle_Theme_Updater( array(
            ...
            'license' 			=> "free", 		// The license key (used get_option above to retrieve from DB)
            'item_name' 		=> $theme_data->get('Name'),	// The name of this theme
            'author'			=> 'ThemeIsle'	// The author's name
        )
    );
}
add_action( 'admin_init', 'shop_isle_theme_updater' );

if(!class_exists('shop_isle_Theme_Updater')) {
    class shop_isle_Theme_Updater {
